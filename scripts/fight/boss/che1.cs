...
public class che1 : MonoBehaviour {
    private new Animation animation;
    public GameObject camera1;
    public int AAA;  //物体攻击力
    ...
    // 0 站姿 WK_catapult      1移动   WK_catapult_02_move     2攻击 WK_catapult_03_attack
    // 3 阵亡 WK_catapult_04_death
    ...
    void Update()
    {
        ...
        if (this.gameObject.GetComponent<value>().hp < 0)
        {
            ...
            if (rand >= 5)
            {
                animation.Play("WK_catapult_04_death");
                if (animation["WK_catapult_04_death"].normalizedTime >= 0.8)
                {
                    Destroy(this.gameObject);//销毁这个物体
                }
            }
        ...
        else
        {
            ...
            if (finda == 1)
            {
                ...
                if (jl > 40100)
                {
                    ...
                    animation.Play("WK_catapult_02_move");//准备战斗
                }
                else
                {
                    animation.Play("WK_catapult_03_attack");
                    if (animation["WK_catapult_03_attack"].normalizedTime < 0.8)
                    {
                        dh = 1;

                    }
                    //Debug.Log(animation["WK_archer_07_attack_A"].normalizedTime);
                    if (animation["WK_catapult_03_attack"].normalizedTime >= 0.8 && dh == 1)
                    {
            ...
            else
            {
                ...
                if (zhuantai == 1)
                {
                    animation.Play("WK_catapult_02_move");
                }
