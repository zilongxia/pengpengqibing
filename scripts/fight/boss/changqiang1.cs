...
public class changqiang1 : MonoBehaviour {
    private new Animation animation;
    public GameObject camera1;
    // private float speed = 0.5f;//速度
    public int AAA = 20;  //物体攻击力
    ...
    // 0 站姿 WK_spearman_01_idle_A   1 走路WK_spearman_02_walk  2跑WK_spearman_03_run
    // 3 冲锋 WK_spearman_04_charge   4 站姿 WK_spearman_05_combat_idle  5 慢走 WK_spearman_06_combat_walk
    // 7 攻击1 WK_spearman_07_attack_A  
    // 8 死亡1，2 WK_spearman_10_death_A  WK_spearman_10_death_B
    ...
    void Update()
    {
        ...
        if (this.gameObject.GetComponent<value>().hp < 0)
        {
            ...
            if (rand >= 5)
            {
                animation.Play("WK_spearman_10_death_A");
                music.clip = bg;
                if (!music.isPlaying)
                {
                    //播放音乐
                ...
                if (animation["WK_spearman_10_death_A"].normalizedTime >= 0.8)
                {
            ...
            if (rand < 5 && rand > 0)
            {
                animation.Play("WK_spearman_10_death_B");
                music.clip = bg2;
                if (!music.isPlaying)
                {
                    //播放音乐
                ...
                if (animation["WK_spearman_10_death_B"].normalizedTime >= 0.8)
                {
                    dest = 1;
                    Destroy(this.gameObject);//销毁这个物体
                }
        ...
        else
        {
            ...
            if (finda == 1)
            {
                ...
                if (jl > 200)
                {
                    ...
                    agent.speed = cc + 5;
                    animation.Play("WK_spearman_04_charge");//准备战斗
                }
                else
                {
                    if (rand1 >= 0 && rand1 < 5)
                    {
                        animation.Play("WK_spearman_07_attack_A");
                        if (animation["WK_spearman_07_attack_A"].normalizedTime < 0.8)
                        {
                            dh = 1;

                        }
                        //Debug.Log(animation["WK_archer_07_attack_A"].normalizedTime);
                        if (animation["WK_spearman_07_attack_A"].normalizedTime >= 0.8 && dh == 1)
                        {
            ...
            else
            {
                ...
                if (zhuantai == 1)
                {
                    animation.Play("WK_spearman_03_run");
                }
