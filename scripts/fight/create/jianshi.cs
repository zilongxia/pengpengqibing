...
public class jianshi : MonoBehaviour {


    private new Animation animation;
    public GameObject camera1;
    // private float speed = 0.5f;//速度
    public int AAA = 5;  //物体攻击力
    ...
    // 0 站姿 WK_heavy_infantry_01_idle_A   1 走路 WK_heavy_infantry_02_walk  2跑 WK_heavy_infantry_03_run
    // 3 冲锋 WK_heavy_infantry_04_charge   4 站姿 WK_heavy_infantry_05_combat_idle  5 慢走 WK_heavy_infantry_06_combat_walk
    // 7 攻击1，2 WK_heavy_infantry_07_attack_A  WK_heavy_infantry_08_attack_B 
    // 8 死亡1，2 WK_heavy_infantry_09_death_A  WK_heavy_infantry_09_death_B
    ...
    void Update()
    {
        ...
        if (this.gameObject.GetComponent<value>().hp < 0)
        {
            ...
            if (rand >= 5)
            {
                animation.Play("WK_heavy_infantry_09_death_A");
                music.clip = bg;
                if (!music.isPlaying)
                {
                    //播放音乐
                ...
                if (animation["WK_heavy_infantry_09_death_A"].normalizedTime >= 0.8)
                {
                    Destroy(this.gameObject);//销毁这个物体
                }
            }
            if (rand < 5 && rand > 0)
            {
                animation.Play("WK_heavy_infantry_09_death_B");
                music.clip = bg2;
                if (!music.isPlaying)
                {
                    //播放音乐
                ...
                if (animation["WK_heavy_infantry_09_death_B"].normalizedTime >= 0.8)
                {
                    dest = 1;
                    Destroy(this.gameObject);//销毁这个物体
                }
        ...
        else
        {
            ...
            for (i = 0; i < b; i++)//找到范围内的敌人
            {
                ...
                if (aa[i].transform.gameObject.tag == "boss")
                {
                    ...
                    //Debug.Log(bossweizhi.Length);
                }
            }
            if (finda == 1)
            {
                
                boss1 = Dpdr(bossweizhi);//判断敌人
                ...
                if (jl > 100)
                {
                    ...
                    agent.speed = cc + 5;
                    animation.Play("WK_heavy_infantry_04_charge");//准备战斗
                }
                else
                {
                    if (rand1 >= 0 && rand1 < 5)
                    {
                        animation.Play("WK_heavy_infantry_07_attack_A");
                        if (animation["WK_heavy_infantry_07_attack_A"].normalizedTime < 0.8)
                        {
                            dh = 1;

                        }
                        //Debug.Log(animation["WK_archer_07_attack_A"].normalizedTime);
                        if (animation["WK_heavy_infantry_07_attack_A"].normalizedTime >= 0.8 && dh == 1)
                        {
                    ...
                    {
                        animation.Play("WK_heavy_infantry_08_attack_B");
                        if (animation["WK_heavy_infantry_08_attack_B"].normalizedTime < 0.8)
                        {
                            dh = 1;

                        }
                        //Debug.Log(animation["WK_archer_07_attack_A"].normalizedTime);
                        if (animation["WK_heavy_infantry_08_attack_B"].normalizedTime >= 0.8 && dh == 1)
                        {
                            //Debug.Log("aaaaaaaaaaaaaaaaaa");
                            boss1.gameObject.GetComponent<value>().hp = boss1.gameObject.GetComponent<value>().hp - AAA;//攻击boss
                            dh = 0;
            ...
            else
            {
                ...
                if (zhuantai == 1)
                {
                    animation.Play("WK_heavy_infantry_03_run");
                }
    ...
    public Transform Dpdr(Transform[] bossweizhi)
    {// 判断敌人
        Transform bossz;
        int i = 0;
        //Debug.Log(bossweizhi.Length);
        for (i = 0; i < bossweizhi.Length - 1; i++)
        {
            ...
            //Debug.Log(Vector3.Distance(bossweizhi[i].position, this.transform.position));
            //Debug.Log(z1);
            if (z < z1)
            {//如果后面的物体小于前面的物体则替换
